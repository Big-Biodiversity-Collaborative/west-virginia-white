# Linear regression analysis for yearly change in julian day; covariate: spring temperature
# Jeff Oliver
...
# Add spring average temperature data from https://climatena.ca/spatialData
tave_sp <- terra::rast(x = "data/Tave_sp_1991-2020.tif")

# Extract relevant data for observations
all_obs$tave_sp <- terra::extract(x = tave_sp,
                              y = all_obs[, c("longitude", "latitude")])[, 2]

################################################################################
# Analyses
# Run linear regression, no interaction
model_1 <- lm(julian_day ~ year + tave_sp + species, 
                data = all_obs)

# Add the interaction between year and species
model_2 <- lm(julian_day ~ tave_sp + year * species,
                      data = all_obs)

# Check to see if this is a better model
anova(model_1, model_2)

# Add the two-way interaction between year and tave_sp, allowing tave_sp-specific 
# responses to year, across all species
model_3 <- lm(julian_day ~ year * species + year * tave_sp,
                  data = all_obs)
# summary(model_3)
anova(model_2, model_3)

# Now add the last two-way interaction between tave_sp and species (species-
# specific responses to local temperatures)
model_4 <- lm(julian_day ~ (year + tave_sp + species)^2,
              data = all_obs)
# summary(model_4)
anova(model_3, model_4)

# Full model includes
model_5 <- lm(julian_day ~ year * tave_sp * species,
                 data = all_obs)
# summary(model_5)
...
write.csv(x = model_compare,
          file = "output/model-compare-tave_sp.csv",
          row.names = FALSE)
...
# data:  best_model
# BP = 358.63, df = 15, p-value < 2.2e-16
...
# BP = 12.959, df = 15, p-value = 0.6054
...
model_table <- model_results %>%
  mutate(term = gsub(x = term, pattern = "tave_sp", replacement = "Temperature")) %>%
  ...
  mutate(term = gsub(x = term, pattern = "tave_sp:", replacement = "Temperature x ")) %>%
...
# Write to a file
write.csv(file = "output/model-table-tave_sp.csv",
          x = model_table,
          row.names = FALSE)

# How many days/year earlier for:
#    + each species
#    + each tave_sp (will be an "average" for each of three categories)
# Full (best) model is
# Julian Day = B0 + B1 x year + B2 x tave_sp + B3 x species +
#                   B4 x year x tave_sp + B5 x year x species + B6 x tave_sp x species +
#                   B7 x year x tave_sp x species
...
# B1 + B5 + tave_sp x (B4 + B7)
# where B5 and B7 have specific values for each host

# Need to have values to substitute in for tave_sp
...
tave_sp_points <- stats::quantile(x = all_obs$tave_sp[all_obs$organism == "insect"],
                              probs = c(1/6, 1/2, 5/6))

# Insect delta days/year:
insect_delta <- model_results$estimate[model_results$term == "year"] +
  (model_results$estimate[model_results$term == "year:tave_sp"] * tave_sp_points)
...
host_deltas <- sapply(X = host_names,
                      FUN = function(x) {
                        ...
                        B4 <- model_results$estimate[model_results$term == "year:tave_sp"]
                        B5 <- model_results$estimate[model_results$term == paste0("year:species", x)]
                        B7 <- model_results$estimate[model_results$term == paste0("year:tave_sp:species", x)]
                        delta <- B1 + B5 + (tave_sp_points * (B4 + B7))
                        return(delta)
                      })
...
write.csv(x = all_deltas,
          file = "output/changes-table-tave_sp.csv",
          row.names = FALSE)
...
write.csv(x = compared_to_insect,
          file = "output/changes-rel-insect-tave_sp.csv",
          row.names = FALSE)

################################################################################
# Plot responses
# Want three sub-plots, one for low temp, medium temp, high temp
...
# Low Temp
low_tave_sp_prediction <- ggplot(data = newdata %>% 
                               filter(tave_sp == tave_sp_points[1]), 
                             mapping = aes(x = year, 
                                           y = julian_day,
                                           color = species)) +
  geom_line(lwd = 1) +
  ...
  scale_color_manual(values = line_colors,
                     name = "Species") +
  theme_bw() +
  labs(title = "Low Temperature", x = "Year", y = "Julian day")
low_tave_sp_prediction
# ggsave(filename = "output/figure-3a.png",
#        plot = low_tave_sp_prediction)

# Medium Temp
medium_tave_sp_prediction <- ggplot(data = newdata %>% 
                                      filter(tave_sp == tave_sp_points[2]), 
                             mapping = aes(x = year, 
                                           y = julian_day,
                                           color = species)) +
  geom_line(lwd = 1) +
  ...
  scale_color_manual(values = line_colors,
                     name = "Species") +
  theme_bw() +
  labs(title = "Medium Temperature", x = "Year", y = "Julian day")
medium_tave_sp_prediction
# ggsave(filename = "output/figure-3b.png",
#        plot = medium_tave_sp_prediction)

# High Temp
high_tave_sp_prediction <- ggplot(data = newdata %>% 
                                    filter(tave_sp == tave_sp_points[3]), 
                                mapping = aes(x = year, 
                                              y = julian_day,
                                              color = species)) +
  geom_line(lwd = 1) +
  ...
  scale_color_manual(values = line_colors,
                     name = "Species") +
  theme_bw() +
  labs(title = "High Temperature", x = "Year", y = "Julian day")
high_tave_sp_prediction
# ggsave(filename = "output/figure-3c.png",
#        plot = high_tave_sp_prediction)

# Single, multi-panel figure
# multi_panel <- ggpubr::ggarrange(low_tave_sp_prediction, 
#                                  medium_tave_sp_prediction, 
#                                  high_tave_sp_prediction,
#                                  ncol = 1)
# multi_panel
