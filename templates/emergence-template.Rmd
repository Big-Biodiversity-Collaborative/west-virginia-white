...
For the simple model, only using year a the predictor, the mean change in emergence times was `r round(x = abs(lm_report$estimate[1]), digits = 3)` days `r lm_report$direction[1]` per year (p `r lm_report$p_value_text[1]`).
...
**But**, when controlling for latitude and a year x latitude interacton, the mean change in emergence times was `r round(x = abs(lm_report$estimate[2]), digits = 3)` days `r lm_report$direction[2]` per year (p `r lm_report$p_value_text[2]`).
...
wls_complex <- lm(yday ~ year + latitude, data = earliest, weights = n_obs)
...
wls_complex_interaction <- lm(yday ~ year + latitude + year * latitude, 
                  data = earliest, 
                  weights = n_obs)
...
```{r}
summary(wls_complex_interaction)
```
...
Yeah, it would be cool to try a linear spline, but not until I figure out a better way of dealing with latitude.
