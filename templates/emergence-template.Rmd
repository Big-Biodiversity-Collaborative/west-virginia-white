...
When controlling for latitude and a year x latitude interacton, the mean change in emergence times was `r round(x = abs(lm_report$estimate[2]), digits = 3)` days `r lm_report$direction[2]` per year (p `r lm_report$p_value_text[2]`).

```{r plot-bootstrap-lm, fig.height = 6}
...
wls_complex <- lm(yday ~ year + latitude + year * latitude, 
                  data = earliest, 
                  weights = n_obs)
wls_complex_summary <- summary(wls_complex)
```
