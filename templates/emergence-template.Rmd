...
bs_results <- parLapply(cl = the_cluster,
                        ...
                        fun = function(i, bs_df, sample_df, sample_size) {
                          ...
                          for (yr in unique(bs_df$year)) {
                            # TODO: revise sampling code so that instead of 
                            # sampling a vector, instead sample a *row*, so we
                            # can have lat/long data, too
                            # sample_vec <- yday(sample_df$date)[year(sample_df$date) == yr]
                            # bs_df$yday[bs_df$year == yr] <- sample(x = sample_vec,
                            #                                        size = sample_size,
                            #                                        replace = FALSE)
                            # Start with subsetting the dataframe to only year
                            # of interest
                          ...
                          # TODO: this needs to be updated so other data come
                          # through (i.e. longitude & latitude)
                          bs_mins <- bs_df %>%
                            group_by(year) %>%
