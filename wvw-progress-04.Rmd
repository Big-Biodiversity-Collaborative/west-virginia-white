...
# TODO: Need to do filtration for each species separately.
year_counts <- all_obs %>%  
...
pred_lats <- c(min_lat_insect, (min_lat_insect + med_lat_insect)/2,
               med_lat_insect,
               (max_lat_insect + med_lat_insect)/2, max_lat_insect)
...
# Interpolation
One approach used in other phenological studies uses interpolation to estimate the day on which 10% of the observations for that year occurred (and 50% and 90%). This approach should be less influenced by sample size. For example, we can look at 